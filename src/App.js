...
class App extends React.Component {
  ...
  changeProductsDisplayHandler = (e) => {
    this.setState({
      displayProducts: e.target.value === "grid" ? "grid" : "list",
    });
  };
  toggleTheme = () => {
  ...
  render() {
    ...
    // price slider filter
    if (
      this.state.minInputRange !== getMinPrice() ||
      this.state.maxInputRange !== getMaxPrice()
    ) {
      console.log(this.state.minInputRange, this.state.maxInputRange);
      filteredProducts = filteredProducts.filter(
        (product) =>
          product.price >= this.state.minInputRange &&
          product.price <= this.state.maxInputRange
      );
    }
    // category filter
    ...
    if (this.state.brand !== "allBrands") {
      filteredProducts = filteredProducts.filter(
        (product) => product.brand === this.state.brand
      );
    }
    // sort
      ...
      <div
        className={
          this.state.darkTheme
            ? `${styles["body__wrapper"]} ${styles["dark"]}`
        ...
        <main className={styles["container"]}>
          <header className={styles["main__header"]}>
            <div
              className={
                this.state.darkTheme
                  ? `${styles["header__path"]} ${styles["dark"]}`
                  : `${styles["header__path"]}`
              }
            >
              <h2>shop</h2>
                ...
                <li>
                  <a href="#">
                    <ChevronRightOutlinedIcon
                      style={{
                        fontSize: "2rem",
                        color: this.state.darkTheme ? "#b4b7bd" : "#6e6b7b",
                      }}
                    />
                  </a>
                </li>
          ...
          <section className={styles["products__section"]}>
            <ProductFilter
              ...
              maxPercentage={
                100 - (this.state.maxInputRange / getMaxPrice()) * 100
              }
